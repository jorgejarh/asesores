...
class Pl_capacitaciones_eval_model extends CI_Model {
	
	public $nombre_tabla="pl_capacitaciones_eval";
	...
	function lista($id=0)
	{
		return $this->db->select("a.*, b.nombre_tipo_evaluacion ",false)->where("a.id_tipo_evaluacion = b.id_tipo_evaluacion")->get_where($this->nombre_tabla." a, mante_tipo_evaluacion b",array('a.activo'=>1))->result_array();
		
	}
	
	function obtener_sum_porcentaje($id_capactacion=0)
	{
		
		$resultado=$this->db->select("SUM(a.porcentaje) as suma ",false)->get_where($this->nombre_tabla." a",array('a.activo'=>1,'a.id_capacitacion'=>$id_capactacion))->row_array();
		
		return $resultado["suma"];
		//SELECT SUM(porcentaje) FROM pl_capacitaciones_eval WHERE id_capacitacion=2 AND activo = 1
	}
