...
class Cooperativa_model extends CI_Model {
    ...
    function obtener_cooperativa($id=0)
    {
		if($id!=0)
		{
        	return $this->db->order_by('cooperativa','ASC')->get_where('conf_cooperativa',array('id_cooperativa'=>$id))->row_array();
		}else{
			return $this->db->order_by('cooperativa','ASC')->get_where('conf_cooperativa',array('activo'=>1))->result_array();
			}
    }
    ...
    function obtener_sucursales($id=0)
    {
		if($id!=0)
		{
        	return $this->db->order_by('sucursal','ASC')->select('a.*,b.cooperativa')->get_where('conf_sucursal a, conf_cooperativa b','a.id_sucursal = '.$id.' and a.id_cooperativa = b.id_cooperativa')->row_array();
		}else{
        	return $this->db->order_by('sucursal','ASC')->select('a.*,b.cooperativa')->get_where('conf_sucursal a, conf_cooperativa b','a.id_cooperativa = b.id_cooperativa and a.activo = 1')->result_array();
			}
