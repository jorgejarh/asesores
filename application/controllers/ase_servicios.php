...
class Ase_servicios extends CI_Controller {

	public $datos_user=array();

	public $carpeta_view="ase_servicios";

	public $modelo_usar="ase_servicios_model";

	public $nombre_controlador="ase_servicios";
	
	public $nombre_titulo="Servicios";
	...
	function __construct()
    {
		...
		$this->set_campo("id_tipo_solicitante","Tipo Solicitante",'required|xss_clean','select',preparar_select($this->$model->obtener_tipos(),'id_tipo_solicitante','tipo_solicitante'));
		$this->set_campo("id_cooperativa","Cooperativa",'xss_clean','select',array());
		$this->set_campo("nombre_solicitante","Solicitante",'required|xss_clean');
	...
	public function obtener_cooperativas()
	{
		$post=$this->input->post();
		...
		if($post)
		{
			if($post['id_tipo']==1)
			{
				...
				$cooperativas= $this->cooperativa_model->obtener_cooperativa();
			}else{
				$cooperativas= array(0=>array(
									'id_cooperativa'=>0,
									'cooperativa'=>'Ninguna'
				));
				}
			echo json_encode($cooperativas);
		}
	}	
